...
class RobberManager:
    """Manages all robber-related functionality in the game."""
    ...
    def draw_robber(self, screen):
        ...
        robber_q, robber_r = self.game.board.axial_layout[self.game.board.robber_position]
    ...
    def draw_placement_indicator(self, screen, mouse_pos: Tuple[int, int]):
        """Draw indicator for valid robber placement locations."""
    ...
    def handle_click(self, pos: Tuple[int, int]) -> bool:
        """Handle all robber-related clicks."""
    ...
    def _handle_tile_click(self, mouse_pos: Tuple[int, int]) -> bool:
        ...
        for idx, (q, r) in enumerate(self.game.board.axial_layout):
            ...
            if math.hypot(mouse_pos[0] - x, mouse_pos[1] - y) <= TILE_SIZE:
                if idx != self.game.board.robber_position:
                    self.game.board.move_robber(idx)
                    self.move_pending = False
    ...
    def _find_potential_victims(self, tile_idx: int) -> List[int]:
        """Find all players who have settlements/cities on the given tile."""
        victims = set()
        ...
        for corner in corners:
            ...
            # check settlements and cities
            for structures, structure_type in [(self.game.settlements, "settlement"), 
                                            (self.game.cities, "city")]:
                if rounded_corner in structures:
                    player_idx = structures[rounded_corner]
                    if (player_idx != self.game.current_player_index and 
    ...
    def _steal_from_player(self, victim_idx: int):
        ...
        victim = self.game.players[victim_idx]
        thief = self.game.players[self.game.current_player_index]
